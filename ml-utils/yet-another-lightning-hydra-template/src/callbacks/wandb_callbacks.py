...
class LogPredictionsCallback(Callback):
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        if batch_idx == 0:
            n = 20
            x, y = batch
            images = [img for img in x.get("image")[:n]]
            captions = [
                f"Ground Truth: {y_i} - Prediction: {y_pred}"
                for y_i, y_pred in zip(y[:n], outputs.get("preds").argmax(dim=1)[:n])
            ]

            # Option 1: log images with `WandbLogger.log_image`
            logger.log_image(key="sample_images", images=images, caption=captions)
            ...
            data = [
                ...
                for x_i, y_i, y_pred in list(
                    zip(
                        x.get("image")[:n],
                        y[:n],
                        outputs.get("preds").argmax(dim=1)[:n],
                    )
                )
            ]
