...
class SemanticKitti(Dataset):
  def __init__(
    ...
    self.transform = sensor["transform"]
    """"""
    ...
    for seq in self.sequences:
      ...
      if self.use_residual:
        residual_path = os.path.join(self.root, seq, "residual_images_1")
      ...
      if self.use_residual:
        residual_files = [
            os.path.join(dp, f) for dp, dn, fn in os.walk(os.path.expanduser(residual_path)) for f in fn if is_residual(f)
        ]
      ...
      if self.use_residual:
        # check all scans have a residual
        assert (len(scan_files) == len(residual_files))
      """
      Added for dynamic object segmentation
      """
      # fill index mapper which is needed when loading several frames
      n_used_files = max(0, len(scan_files) - self.n_input_scans + 1)
  ...
  def __getitem__(self, dataset_index):
    ...
    current_index = start_index + self.n_input_scans - 1
    ...
    for index in range(start_index, start_index + self.n_input_scans):
      ...
      if self.use_residual:
        residual_file = self.residual_files[seq][index]
      ...
      scan.open_scan(scan_file, index_pose, current_pose, if_transform=self.transform)
      ...
      # get points and labels for n_input_scans input scanes
      ...
      if self.use_residual:
        proj_residuals = torch.Tensor(np.load(residual_file))
    ...
    # add residuals channel
    if self.use_residual:
      proj_full = torch.cat([proj_full, torch.unsqueeze(proj_residuals, 0)])
