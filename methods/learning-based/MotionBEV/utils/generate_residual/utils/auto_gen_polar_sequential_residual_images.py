        ...
        if frame_idx < num_prev_n + num_last_n - 1:
            ...
            if frame_idx == num_prev_n + num_last_n - 1:  # initialize
                for i in range(-num_last_n - num_prev_n + 1, -num_last_n + 1):
                    ...
                    prev_scan_transformed = (
                        np.linalg.inv(current_pose).dot(prev_pose).dot(prev_scan.T).T
                    )
                    if prev_sub_map is None:
                        prev_sub_map = prev_scan_transformed
                        prev_diff_map = np.full(
                            (prev_scan.shape[0], num_prev_n + num_last_n),
                            0,
                            dtype=np.float32,
                        )
                        prev_index = np.full(prev_scan.shape[0], num_last_n, dtype=int)
                    else:
                        prev_sub_map = np.concatenate(
                            (prev_sub_map, prev_scan_transformed), axis=0
                        )
                        ...
                        prev_index = np.concatenate(
                            (
                                prev_index,
                                np.full(prev_scan.shape[0], num_last_n, dtype=int),
                            ),
                            axis=0,
                        )
                ...
                for i in range(-num_last_n + 1, 1):
                    ...
                    if last_sub_map is None:
                        ...
                        last_diff_map = np.full(
                            (last_scan.shape[0], num_prev_n + num_last_n),
                            0,
                            dtype=np.float32,
                        )
                        last_index = np.full(last_scan.shape[0], 0, dtype=int)
                    else:
                        last_sub_map = np.concatenate(
                            (last_sub_map, last_scan_transformed), axis=0
                        )
                ...
                prev_sub_map = (
                    np.linalg.inv(current_pose).dot(last_pose).dot(prev_sub_map.T).T
                )
                last_sub_map = (
                    np.linalg.inv(current_pose).dot(last_pose).dot(last_sub_map.T).T
                )
                ...
                last_diff_map = np.concatenate(
                    (
                        last_diff_map,
                        np.full(
                            (current_scan.shape[0], num_prev_n + num_last_n),
                            0,
                            dtype=np.float32,
                        ),
                    ),
                ...
                last_index = np.concatenate(
                    (last_index, np.full(current_scan.shape[0], 0, dtype=int)), axis=0
                )
            ...
            (
                ...
                current_vertex=prev_sub_map.astype(np.float32),
            ...
            (
                ...
                current_vertex=last_sub_map.astype(np.float32),
                proj_H=polar_image_params["height"],
                proj_W=polar_image_params["width"],
                max_range=polar_image_params["max_range"],
                min_range=polar_image_params["min_range"],
            ...
            if config["occlusion_block"]:
                residual_proj_z_delta[last_occlusion_mask] = (
                    0  # residual_proj_z_delta[last_proj_z_delta == 0] = 0
                )
            prev_diff_map[
                prev_mask_valid, prev_index[prev_mask_valid]
            ] += residual_proj_z_delta[prev_proj_y, prev_proj_x]
            ...
            if occlusion_block:
                residual_proj_z_delta[prev_occlusion_mask] = (
                    0  # residual_proj_z_delta[prev_proj_z_delta == 0] = 0
                )
            last_diff_map[
                last_mask_valid, last_index[last_mask_valid]
            ] += residual_proj_z_delta[last_proj_y, last_proj_x]
            ...
            prev_sub_map = np.concatenate(
                (prev_sub_map, last_sub_map[:last_scan_size]), axis=0
            )
            last_sub_map = last_sub_map[last_scan_size:]
            prev_diff_map = np.concatenate(
                (prev_diff_map, last_diff_map[:last_scan_size]), axis=0
            )
            last_diff_map = last_diff_map[last_scan_size:]
            prev_index = np.concatenate(
                (prev_index, last_index[:last_scan_size]), axis=0
            )
            last_index = last_index[last_scan_size:]

            # save
            file_name = os.path.join(
                residual_image_folder,
                str(frame_idx - num_last_n - num_prev_n + 1).zfill(6),
            )
            np.save(file_name, diff_scan)

    print("Saving last few files...")
    for frame_idx in range(
        len(scan_paths) - num_last_n - num_prev_n + 1, len(scan_paths)
    ):
...
if __name__ == "__main__":
    ...
    for seq in sequences:  # sequences id
        ...
        config["residual_image_folder"] = (
            residual_image_folder + f"{'%02d' % seq}/residual_images"
        )
        ic(config)
