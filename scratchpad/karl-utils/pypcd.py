...
class PointCloud(object):
    ...
    def add_labels(self, labels, field_name="label", dtype=np.int32):
        assert labels.shape[0] == self.pc_data.shape[0]

        # Update pc data
        self.pc_data = rec_append_fields(
            self.pc_data, field_name, labels, dtypes=[dtype]
        )
        ...
        md["fields"] = self.pc_data.dtype.names
        for field in md["fields"]:
            type_, size_ = numpy_type_to_pcd_type[self.pc_data.dtype.fields[field][0]]
            md["type"].append(type_)
            md["size"].append(size_)
            # TODO handle multicount
            md["count"].append(1)
        md["width"] = len(self.pc_data)
        md["points"] = len(self.pc_data)
        self.metadata_keys = md.keys()
    ...
    def save(self, fname):
        with open(fname, "w") as f:
            point_cloud_to_fileobj(self, f, "ascii")
